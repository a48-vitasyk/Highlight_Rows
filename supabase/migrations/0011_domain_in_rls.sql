-- Highlight Rows — фаза 11: перевірка домену @zomro.org у самих RLS-політиках.
...
create policy "read" on public.reminders for select to authenticated
    using ( auth.email() ~* '@zomro\.org$' and (scope = 'shared' or created_by = auth.uid()) );
create policy "insert" on public.reminders for insert to authenticated
    with check ( auth.email() ~* '@zomro\.org$' and created_by = auth.uid() );
...
create policy "snip write" on public.snippets for all to authenticated
    using ( auth.email() ~* '@zomro\.org$' ) with check ( auth.email() ~* '@zomro\.org$' );
